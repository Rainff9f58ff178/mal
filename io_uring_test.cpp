...
void _send_static_string_content(const char *str, int client_socket) {
    struct request *req = (request*)zh_malloc(sizeof(*req) + sizeof(struct iovec));
...
void handle_get_method(char *path, int client_socket) {
    ...
    if (stat(final_path, &path_stat) == -1) {
        ...
        if (S_ISREG(path_stat.st_mode)) {
            struct request *req =(request*) zh_malloc(sizeof(*req) + (sizeof(struct iovec) * 6));
            req->iovec_count = 6;
            req->client_socket = client_socket;
...
//    server_loop(server_socket);
    ...
    io_uring ring_;
    io_uring_queue_init(INT16_MAX,&ring_,0);

    auto entry = io_uring_get_sqe(&ring_);
    auto s_e= io_uring_get_sqe(&ring_);
    int file_fd= open("./src/test.js",O_RDONLY)
            ;
    io_uring_prep_read(entry,file_fd,buf,sizeof buf,0);
    io_uring_sqe_set_data(entry,buf);
    io_uring_prep_read(s_e,file_fd,buf,sizeof buf,0);
    io_uring_sqe_set_data(s_e,buf);

    io_uring_cqe* c_e_;
    io_uring_submit(&ring_);
    auto e = io_uring_wait_cqe(&ring_,&c_e_);
    if(e==0){
        while(io_uring_peek_cqe(&ring_,&c_e_)==0){
            printf("still");
        }
    }
    return 0;
